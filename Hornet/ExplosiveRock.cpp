...
const double ACTIVE_RANGE = 1000;
...
void ExplosiveRock::Initialise()
{
    ...
    m_explosiveRockSpeed = rand() % 51 + 150;
...
void ExplosiveRock::Update(double frametime)
{
    ...
    if (m_pTarget->IsActive() == true)
    {
        ...
        m_distance = m_playerLocation.magnitude();
...
{
    //No sound becuase running out of channels during gameplay
    if (other.GetType() == ObjectType::EXPLOSIVEROCK)
    {
        Deactivate();
